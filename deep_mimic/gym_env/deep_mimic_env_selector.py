...
from deep_mimic.env.pybullet_deep_mimic_env_selector import PyBulletDeepMimicEnvSelector, InitializationStrategy
from deep_mimic.gym_env.deep_mimic_env import HumanoidDeepBulletEnv
...
class HumanoidDeepBulletSelectorEnv(HumanoidDeepBulletEnv):
    ...
    def __init__(self, renders=False, arg_file='', test_mode=False,
        ...
        observation_min = np.array([0.0]+[-100.0]+[-4.0]*105+[-500.0]*90+clips_min)
        observation_max = np.array([1.0]+[100.0]+[4.0]*105+[500.0]*90+clips_max)
    ...
    def reset(self):
        ...
        if self._internal_env is None:
            ...
            self._internal_env = PyBulletDeepMimicEnvSelector(self._arg_parser, self._renders,
...
class HumanoidDeepMimicWalkerBulletEnv(HumanoidDeepBulletSelectorEnv):
    metadata = {'render.modes': ['human', 'rgb_array'], 'video.frames_per_second': 50}

    def __init__(self, renders=False):
        # start the bullet physics server
        HumanoidDeepBulletSelectorEnv.__init__(self, renders, arg_file="run_humanoid3d_walker_args.txt")

class HumanoidDeepMimicDancerBulletEnv(HumanoidDeepBulletSelectorEnv):
    metadata = {'render.modes': ['human', 'rgb_array'], 'video.frames_per_second': 50}

    def __init__(self, renders=False):
        # start the bullet physics server
        HumanoidDeepBulletSelectorEnv.__init__(self, renders, arg_file="run_humanoid3d_dancer_args.txt")

class HumanoidDeepMimicSignerBulletEnv(HumanoidDeepBulletSelectorEnv):
    metadata = {'render.modes': ['human', 'rgb_array'], 'video.frames_per_second': 50}

    def __init__(self, renders=False):
        # start the bullet physics server
        HumanoidDeepBulletSelectorEnv.__init__(self, renders, arg_file="run_humanoid3d_signer_args.txt")