...
from deep_mimic.env import hand_stable_pd
...
class PyBulletDeepMimicEnv(Env):
  ...
  def reset(self):
    ...
    if not self._isInitialized:
      ...
      useFixedBase = False
      self._hand = hand_stable_pd.HandStablePD(self._pybullet_client, self._mocapData,
    ...
    if self._init_strategy == InitializationStrategy.RANDOM:
      ...
      startTime = float(rn) / rnrange * self._hand.getCycleTime()
    elif self._init_strategy == InitializationStrategy.START:
      startTime = 0
    
    self.t = startTime

    self._hand.setSimTime(startTime)

    self._hand.resetPose()
  ...
  def get_action_size(self, agent_id):
    ctrl_size = 43  #numDof TODO check numDof for hands
    root_size = 7
  ...
  def build_action_offset(self, agent_id):
    ...
    out_offset = [
        ...
        0.00000000, 0.00000000, 0.00000000, -0.2000000, 0.00000000, 0.00000000, 0.00000000,
        -0.2000000, -1.5700000 # TODO change values
    ]
  ...
  def build_action_scale(self, agent_id):
    ...
    out_scale = [
        ...
        0.159235668789 # TODO change values
    ]
    return np.array(out_scale)
  ...
  def build_action_bound_min(self, agent_id):
    ...
    out_scale = [
        ...
        -1.000000000, -4.710000000 # TODO change values
    ]
  ...
  def build_action_bound_max(self, agent_id):
    ...
    out_scale = [
        ...
        1.0000000, 10.100000, 1.0000000, 1.0000000, 1.0000000, 7.8500000 # TODO change values
    ]
    return out_scale
  ...
  def calc_reward(self, agent_id):
    kinPose = self._hand.computePose(self._hand._frameFraction)
    reward = self._hand.getReward(kinPose)
    return reward

  def set_action(self, agent_id, action):
    self.desiredPose = self._hand.convertActionToPose(action)
  ...
  def update(self, timeStep):
    self._pybullet_client.setTimeStep(timeStep)
    self._hand._timeStep = timeStep
    self.timeStep = timeStep

    for i in range(1):
      self.t += timeStep
      self._hand.setSimTime(self.t)

      if self.desiredPose:
        kinPose = self._hand.computePose(self._hand._frameFraction)
        self._hand.initializePose(self._hand._poseInterpolator,
                                  self._hand._kin_model,
                                  initBase=True)
        ...
        maxForces = [
            ...
            100, 100, 60 # TODO change values
        ]

        if self._useStablePD:
          self._hand.computeAndApplyPDForces(self.desiredPose,
                                               maxForces=maxForces)
        else:
          self._hand.setJointMotors(self.desiredPose, maxForces=maxForces)
...
if __name__ == '__main__':
  ...
  env = PyBulletDeepMimicEnv(arg_parser, True)
  import time
  time.sleep(50)