...
class MotionCaptureDataMultiClip(object):
    ...
    def getKeyFrameDuration(self, i):
        return self._motion_data[i]['Frames'][0][0]

    def getCycleTime(self, i):
        keyFrameDuration = self.getKeyFrameDuration(i)
    ...
    def computeCycleOffset(self, i):
        ...
        frameData = self._motion_data[i]['Frames'][0]
        frameDataNext = self._motion_data[i]['Frames'][lastFrame]
    ...
    def deepmimic_to_scipy_quaternion(self, quat):
        return quat[1:] + [quat[0]] # [x, y, z, w] -> [w, x, y, z]

    def scipy_to_deepmimic_quaternion(self, quat):
        return [quat[-1]] + quat[:-1] # [w, x, y, z] -> [x, y, z, w]
    ...
    def translate_root_pos(self, new_clip_num, current_root):
        prev_frame_root = self._motion_data[new_clip_num]["Frames"][0][1:4]
        self._motion_data[new_clip_num]["Frames"][0][1] = current_root[0]
        self._motion_data[new_clip_num]["Frames"][0][3] = current_root[2]
        for i in range(1, len(self._motion_data[new_clip_num]["Frames"])):
            curr_frame_root = self._motion_data[new_clip_num]["Frames"][i][1:4]
            root_diff = [c - p for c, p in zip(curr_frame_root, prev_frame_root)]
            new_root = [np + d for np, d in zip(self._motion_data[new_clip_num]["Frames"][i-1][1:4], root_diff)]
            self._motion_data[new_clip_num]["Frames"][i][1] = new_root[0]
            self._motion_data[new_clip_num]["Frames"][i][3] = new_root[2]
            prev_frame_root = curr_frame_root

    def translate_root_rot(self, new_clip_num, head_offset):
        scipy_offset = R.from_euler("xyz", [0, -head_offset, 0], degrees=False)
        for i in range(len(self._motion_data[new_clip_num]["Frames"])):
            curr_frame_rot = self._motion_data[new_clip_num]["Frames"][i][4:8]
            curr_frame_rot = self.deepmimic_to_scipy_quaternion(curr_frame_rot)
            curr_frame_rot = R.from_quat(curr_frame_rot)
            new_rot = scipy_offset * curr_frame_rot
            new_rot = new_rot.as_quat().tolist()
            self._motion_data[new_clip_num]["Frames"][i][4:8] = self.scipy_to_deepmimic_quaternion(new_rot)